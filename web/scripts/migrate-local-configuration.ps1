...
if (-not (Test-Path -LiteralPath $runtimeEnvPath)) {
  ...
  $lines = @(
    ...
    "CANDIDATE_TOOL_EXECUTION_MODE=$(& $value 'CANDIDATE_TOOL_EXECUTION_MODE' 'production')", "CANDIDATE_PIPELINE_ROUTING=$(& $value 'CANDIDATE_PIPELINE_ROUTING' 'shadow')", "CANDIDATE_PIPELINE_BUILD_ID=$(& $value 'CANDIDATE_PIPELINE_BUILD_ID' 'local-unprovisioned')",
