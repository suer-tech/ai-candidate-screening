...
export async function POST(request: Request) {
  ...
  try {
    const payload = await request.json() as { candidateId: number; action: "archive" | "restore" | "delete" | "reprocess"; expectedRevision: number };
    const { productRepository } = await import("../../../../server/product/runtime-bindings.ts");
    const candidate = await executeLifecycleCommand(productRepository(), { ...payload, actor });
