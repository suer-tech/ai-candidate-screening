...
export class PostgresVacancyMatrixRepository {
  ...
  async claimCompilation(input: { profileVersion: string; ownerId: string; now: Date; leaseMs: number; allowRetry?: boolean }) {
    return withTransaction(this.sql, async (transaction) => {
      const rows = await transaction<{ state: string; owner_id: string; fencing_token: number; lease_expires_at_utc: Date | string; matrix_id: string | null; attempt: number; terminal_error_code: string | null }[]>`
        ...
        FROM vacancy_matrix_compilations WHERE profile_version=${input.profileVersion} FOR UPDATE`;
      ...
      if (!current) {
        await transaction`INSERT INTO vacancy_matrix_compilations
          (profile_version,state,owner_id,fencing_token,lease_expires_at_utc,attempt)
          VALUES (${input.profileVersion},'CLAIMED',${input.ownerId},1,${leaseExpiresAt},1)`;
      ...
      await transaction`UPDATE vacancy_matrix_compilations SET state='CLAIMED',owner_id=${input.ownerId},fencing_token=${fencingToken},
        ...
        same_fingerprint_retries=0,terminal_error_code=NULL,updated_at_utc=clock_timestamp() WHERE profile_version=${input.profileVersion}`;
      return { owner: true, waiting: false, fencingToken, attempt, recovered: true };
    });
  }

  async publishMatrix(input: { ownerId: string; fencingToken: number; matrix: VacancyMatrix; modelVersions: Record<string, string>; protectedTraceRefs: string[] }) {
    return withTransaction(this.sql, async (transaction) => {
      const rows = await transaction<{ state: string; owner_id: string; fencing_token: number; matrix_id: string | null }[]>`
        ...
        WHERE profile_version=${input.matrix.profileVersion} FOR UPDATE`;
      const current = rows[0];
      ...
      const matrixId = `matrix-${input.matrix.checksum.slice(0, 24)}`;
      await transaction`INSERT INTO vacancy_matrices
        (id,profile_version,schema_version,compiler_policy_version,skill_versions_json,model_versions_json,protected_trace_refs_json,payload_json,checksum)
        VALUES (${matrixId},${input.matrix.profileVersion},${input.matrix.schemaVersion},${input.matrix.compilerPolicyVersion},
      ...
      await transaction`UPDATE vacancy_matrix_compilations SET state='PUBLISHED',matrix_id=${matrixId},updated_at_utc=clock_timestamp()
        WHERE profile_version=${input.matrix.profileVersion}`;
      return { matrixId, checksum: input.matrix.checksum, reused: false };
    });
  }

  async recordCompilationProgress(input: { profileVersion: string; ownerId: string; fencingToken: number; repairCycles: number; llmCalls: number; obstacleFingerprint?: string; sameFingerprintRetries?: number }) {
    const result = await this.sql`UPDATE vacancy_matrix_compilations SET repair_cycles=${input.repairCycles},llm_calls=${input.llmCalls},
      ...
      WHERE profile_version=${input.profileVersion} AND owner_id=${input.ownerId} AND fencing_token=${input.fencingToken} AND state='CLAIMED'`;
    if (Number(result.count) !== 1) throw new Error("MATRIX_STALE_FENCING_TOKEN");
  }

  async failCompilation(input: { profileVersion: string; ownerId: string; fencingToken: number; errorCode: string }) {
  ...
  async readMatrix(profileVersion: string) {
    const rows = await this.sql<{ id: string; payload_json: string; checksum: string }[]>`SELECT id,payload_json,checksum FROM vacancy_matrices WHERE profile_version=${profileVersion}`;
