...
test("production Docker worker accepts only the exact internal web tool endpoint over plaintext", () => {
  const previous = process.env.HH_DOCKER_NETWORK;
  process.env.HH_DOCKER_NETWORK = "1";
  try {
    assert.doesNotThrow(() => createHttpToolAdapters({ endpoint: "http://web:3000/api/internal/candidate-pipeline/tool", token: "x", environment: "production" }));
    assert.throws(() => createHttpToolAdapters({ endpoint: "http://web:3001/api/internal/candidate-pipeline/tool", token: "x", environment: "production" }), /REMOTE_TOOL_ENDPOINT_MUST_USE_HTTPS/);
    assert.throws(() => createHttpToolAdapters({ endpoint: "http://web:3000/api/internal/other", token: "x", environment: "production" }), /REMOTE_TOOL_ENDPOINT_MUST_USE_HTTPS/);
    assert.throws(() => createHttpToolAdapters({ endpoint: "http://web.attacker.invalid:3000/api/internal/candidate-pipeline/tool", token: "x", environment: "production" }), /REMOTE_TOOL_ENDPOINT_MUST_USE_HTTPS/);
  } finally {
    if (previous === undefined) delete process.env.HH_DOCKER_NETWORK;
    else process.env.HH_DOCKER_NETWORK = previous;
  }
});
