...
- `web/server/candidate-pipeline/core.ts` различает interview source `recording` и `ready-transcript` и включает все найденные источники интервью в immutable manifest. `transcription.ts` преобразует каждую запись и каждую TXT/Markdown/SRT/VTT либо извлечённую из transcript-like DOCX стенограмму, затем объединяет их в тот же `transcript-bundle/v1`. Каждая реплика сохраняет `sourceFileId`, версию, имя файла и собственный time/line locator, поэтому одинаковые таймкоды разных интервью не конфликтуют. DOCX рекомендаций и характеристик остаётся supported additional document. Ready-transcript path не вызывает media processor и AssemblyAI; DOCX использует document processor/Mammoth, downstream и selective recovery используют прежний transcription artifact contract.
...
- `web/server/media-processor/`, `web/server/document-processor/` — loopback-only тяжёлые обработчики с отдельными bearer tokens. Media processor принимает исходную запись потоково во временный файл и потоково возвращает извлечённое аудио, поэтому размер видео не дублируется целиком в его Node heap; VPS по умолчанию исполняет только один media shard одновременно, тогда как остальные очереди сохраняют независимую параллельность.
...
Heartbeat executor отправляет не более одного bounded запроса на задачу. Транспортная ошибка или произвольный HTTP 422 не означают потерю lease: остановка происходит при явном `STALE_LEASE_TOKEN` либо истечении последнего подтверждённого срока. Отменённый executor не отправляет terminal outcome, даже если HTTP adapter вернул `FAILED` после abort. Pending heartbeat отменяется при завершении задачи. Publisher периодически вызывает существующий PostgreSQL stale-lease recovery, включая идемпотентное восстановление dispatch; terminal failed runs не перезапускаются. Диагностика heartbeat содержит task/run/attempt identity, HTTP status и безопасную классификацию; неожиданные heartbeat infrastructure errors возвращают 503 без исходного текста исключения. Это не устраняет автоматически причину задержек API: реальный parallel recovery требует отдельной provisioned проверки.

Assessment join использует отдельный `matrix_assessment_summary` с ответом `candidate-assessment-summary/v1`: только рекомендация и обоснование, без повторной генерации уже принятых строк и ABC. `assessment-join.ts` сохраняет исходные rows/ABC/evidence/trace lineage в прежнем bundle v3; downstream snapshot фиксирует фактическую версию summary prompt/schema. Три durable попытки с backoff 5/15 секунд разрешены для transport failures и явно подтверждённого `output_length_exceeded`; provider retry не умножает бюджет обрыва по длине. `provider_content_filter`, refusal и невалидные/неполные ответы без явного length reason завершаются без автоматического повтора. Это согласованная коррекция active change; main OPS-003 требует отдельной синхронизации перед нормативной приёмкой.
...
Промежуточное аудио транскрибации хранится отдельным bounded blob-kind `transcript-audio` с лимитом 256 MiB; обычные domain artifacts сохраняют лимит 8 MiB, а незарегистрированные blob-kind — 32 MiB. Forward-only migration повторно ставит в очередь только активные media-shards, ранее завершившиеся `BLOB_SIZE_LIMIT_EXCEEDED`. Terminal failure обязательного shard переводит fan-out group в `FAILED`, немедленно разрешает join сформировать typed failure и завершает только соответствующий candidate run вместо бесконечного `ACTIVE`.
...
По последнему решению владельца от 2026-09-22 Docker Compose и шаблоны runtime используют `openai/gpt-5.6-luna-pro` вместо Grok как модель по умолчанию. Compose явно передаёт `ROUTERAI_MODEL`, поэтому default перекрывает прежнее значение в mounted `runtime.env`; явный `ROUTERAI_MODEL` в Compose `.env` или окружении оператора имеет приоритет над default. Обычный `docker compose restart` не обновляет environment существующего контейнера: требуется recreation после завершения активных запусков. Выбор относится ко всем текущим LLM capabilities, но не к AssemblyAI. Публичный каталог RouterAI `/api/v1/models` объявляет для этой модели `structured_outputs`, `response_format`, `max_tokens` и `max_completion_tokens`, но не `temperature`: effective configuration для точного ID Luna Pro не задаёт temperature; для остальных моделей сохраняется прежний default `temperature: 0`. Это не заменяет реальный provider smoke и четыре обязательных E2E. Промпты, лимиты, тайм-ауты и основной нормативный контракт этим выбором не меняются. Исправление cooperative evidence planning сохраняется. Реальные secrets и файлы конфигурации сервера этим коммитом не изменяются.
