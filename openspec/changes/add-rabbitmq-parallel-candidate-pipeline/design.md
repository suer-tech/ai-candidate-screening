...
Owner-approved exception to OPS-003: an explicit provider output-length termination may retry at most twice, with the existing 5/15-second task backoff. Network/timeout/429/5xx remain retryable. Schema-invalid, missing, generically incomplete, refusal and content-filter responses are terminal; do not infer length truncation from invalid JSON. Separate safe error codes must distinguish these causes without logging content. Assessment-join owns three durable task attempts and one provider call per attempt, so retry layers do not multiply. Successful upstream artifacts are not regenerated by a task retry. Deployment/E2E remain unverified until exercised in the provisioned environment.
